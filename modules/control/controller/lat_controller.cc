...
Status LatController::Init(std::shared_ptr<DependencyInjector> injector,
  ...
  */
  ...
  matrix_a_(2, 3) = (1.0);
  matrix_a_(3, 2) = ((2.0*lf_ * cf_ - 2.0*lr_ * cr_) / iz_);

  matrix_a_coeff_ = Matrix::Zero(matrix_size, matrix_size);
  matrix_a_coeff_(1, 1) = ((-1.0 * (2.0*c_f + 2.0*c_r) / mass_));
  matrix_a_coeff_(1, 3) = ((2.0*lr_ * cr_ - 2.0*lf_ * cf_) / mass_);
  matrix_a_coeff_(3, 1) = (((2.0*lr_ * cr_ - 2.0*lf_ * cf_) / iz_));
  matrix_a_coeff_(3, 3) = ((-1.0 * (2.0*lf_^2 * cf_ + 2.0*lr_^2 * cr_) / iz_));
